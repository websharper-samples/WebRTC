...
{
 "use strict";
 var Global,Site,Client,Samples,Set,EventTarget,WindowOrWorkerGlobalScope,WebSharper,JavaScript,JS,Sample,Operators,Obj,UI,HtmlModule,attr,List,Doc,SC$1,T,Node,Unchecked,AttrProxy,Pervasives,AttrModule,Array,Client$1,Templates,MinMaxTimeDomain,ASCIICamera,Application,Attrs,DomUtility,View,Arrays,Object,Collections,Dictionary,Seq,SC$2,SC$3,Snap,DocElemNode,CharacterData,Enumerator,SC$4,HashSet,SC$5,Builder,Var$1,T$1,Elt,Attrs$1,Dyn,DictionaryUtil,Prepare,Slice,KeyCollection,AudioHolder,Var,ConcreteVar,Camera,Docs,Updates,Strings,An,Settings,Abbrev,Mailbox,Fresh,Element,HTMLElement,Video,HTMLMediaElement,Ascii,SC$6,Docs$1,RunState,NodeSet,Anims,SC$7,SC$8,AudioNode,ArrayBufferView,Color,HashSetUtil,Queue,SC$9,Concurrency,AppendList,SC$10,Easing,AsyncBody,SC$11,CT,HashSet$1,Scheduler,CancellationTokenSource,DomNodes,Error,OperationCanceledException,Lazy,SC$12,LazyExtensionsProxy,LazyRecord,IntelliFactory,Runtime,Math,console,$,navigator,URL,Date;
 Global=self;
 ...
 Doc=UI.Doc=UI.Doc||{};
 SC$1=Global.StartupCode$WebRTCSamples$Client=Global.StartupCode$WebRTCSamples$Client||{};
 T=List.T=List.T||{};
 Node=Global.Node;
 ...
 MinMaxTimeDomain=Site.MinMaxTimeDomain=Site.MinMaxTimeDomain||{};
 ASCIICamera=Site.ASCIICamera=Site.ASCIICamera||{};
 Application=ASCIICamera.Application=ASCIICamera.Application||{};
 ...
 SC$2=Global.StartupCode$WebRTCSamples$MinMaxTimeDomain=Global.StartupCode$WebRTCSamples$MinMaxTimeDomain||{};
 SC$3=Global.StartupCode$WebRTCSamples$ASCIICamera=Global.StartupCode$WebRTCSamples$ASCIICamera||{};
 ...
 Camera=ASCIICamera.Camera=ASCIICamera.Camera||{};
 ...
 Video=ASCIICamera.Video=ASCIICamera.Video||{};
 HTMLMediaElement=Global.HTMLMediaElement;
 Ascii=ASCIICamera.Ascii=ASCIICamera.Ascii||{};
 ...
 SC$8=Global.StartupCode$WebRTCSamples$AudioHolder=Global.StartupCode$WebRTCSamples$AudioHolder||{};
 AudioNode=Global.AudioNode;
 ArrayBufferView=Global.ArrayBufferView;
 Color=Ascii.Color=Ascii.Color||{};
 ...
 SC$11=Global.StartupCode$WebSharper_Main$Concurrency=Global.StartupCode$WebSharper_Main$Concurrency||{};
 ...
 SC$1.$cctor=function()
 {
  ...
  SC$1.All=Set.Create([Set.Singleton(MinMaxTimeDomain.Sample()),Set.Singleton(Application.Sample())]);
 };
 ...
 MinMaxTimeDomain.Sample=function()
 {
  SC$2.$cctor();
  return SC$2.Sample;
 };
 MinMaxTimeDomain.context=function()
 {
  SC$2.$cctor();
  return SC$2.context;
 };
 MinMaxTimeDomain.Main=function(elem)
 {
  var error,a;
  AudioHolder.StopCurrent();
  error=Var$1.Create$1("");
  a=Doc.BindView(function(a$1)
  {
   return a$1===""?MinMaxTimeDomain.CanvasEl():Doc.Element("div",[],[Doc.TextNode(a$1)]);
  },error.get_View());
  Templates.LoadLocalTemplates("");
  Doc.Run(elem,a);
  !function(t)
  {
   MinMaxTimeDomain.um();
   navigator.getUserMedia(t[0],t[1],t[2]);
  }?error.Set("Your browser does not support this feature!"):MinMaxTimeDomain.LoadSound(function()
  {
   error.Set("No microphone was found!");
  });
 };
 MinMaxTimeDomain.CanvasEl=function()
 {
  SC$2.$cctor();
  return SC$2.CanvasEl;
 };
 MinMaxTimeDomain.um=function()
 {
  SC$2.$cctor();
  return SC$2.um;
 };
 MinMaxTimeDomain.LoadSound=function(onNotSupported)
 {
  MinMaxTimeDomain.um();
  navigator.getUserMedia({
   video:false,
   audio:true
  },MinMaxTimeDomain.Analyser,onNotSupported);
 };
 MinMaxTimeDomain.Analyser=function(stream)
 {
  var analyser,streamNode;
  analyser=MinMaxTimeDomain.context().createAnalyser();
  analyser.smoothingTimeConstant=0;
  analyser.fftSize=1024;
  self.sourceNode=MinMaxTimeDomain.javascriptNode();
  MinMaxTimeDomain.javascriptNode().connect(MinMaxTimeDomain.context().destination);
  MinMaxTimeDomain.javascriptNode().onaudioprocess=function()
  {
   var array;
   array=new Global.Uint8Array(Operators.toInt(analyser.frequencyBinCount));
   analyser.getByteTimeDomainData(array);
   return MinMaxTimeDomain.DrawTimeDomain(MinMaxTimeDomain.CanvasEl().elt.getContext("2d"),array);
  };
  streamNode=MinMaxTimeDomain.context().createMediaStreamSource(stream);
  streamNode.connect(analyser);
  streamNode.connect(MinMaxTimeDomain.context().destination);
  AudioHolder.SetCurrent(streamNode,MinMaxTimeDomain.javascriptNode());
  analyser.connect(MinMaxTimeDomain.javascriptNode());
 };
 MinMaxTimeDomain.javascriptNode=function()
 {
  SC$2.$cctor();
  return SC$2.javascriptNode;
 };
 MinMaxTimeDomain.DrawTimeDomain=function(ctx,array)
 {
  var c,width,height,minValue,maxValue,max,min;
  c=$(MinMaxTimeDomain.CanvasEl().elt);
  width=c.width();
  height=c.height();
  minValue=[9999999];
  maxValue=[0];
  List.iteri(function(i,a)
  {
   var value;
   value=a/256;
   value>maxValue[0]?maxValue[0]=value:void 0;
   return value<minValue[0]?void(minValue[0]=value):null;
  },MinMaxTimeDomain.ToList(array));
  max=height-height*maxValue[0]-1;
  min=height-height*minValue[0]-1;
  ctx.fillStyle="#ffffff";
  ctx.fillRect(MinMaxTimeDomain.column(),min,1,max-min);
  MinMaxTimeDomain.set_column(MinMaxTimeDomain.column()+1);
  MinMaxTimeDomain.column()>=width?(MinMaxTimeDomain.set_column(0),ctx.clearRect(0,0,width,height)):void 0;
 };
 MinMaxTimeDomain.ToList=function(from)
 {
  var n,res,n$1;
  n=from.length-1;
  res=T.Empty;
 ...
 MinMaxTimeDomain.column=function()
 {
  SC$2.$cctor();
  return SC$2.column;
 };
 MinMaxTimeDomain.set_column=function($1)
 {
  SC$2.$cctor();
  SC$2.column=$1;
 };
 Application.Sample=function()
 {
  SC$3.$cctor();
  return SC$3.Sample;
 };
 Application.Main=function(elem)
 {
  var error,asciiContainer,a;
  AudioHolder.StopCurrent();
  ASCIICamera.SetupRequestAnimFrame();
  error=Var$1.Create$1("");
  asciiContainer=Doc.Element("pre",[AttrProxy.Create("style",Application.style())],[Doc.TextView(Application.txt().get_View())]);
  a=Doc.BindView(function(a$1)
  {
   return a$1===""?asciiContainer:Doc.Element("div",[],[Doc.TextNode(a$1)]);
  },error.get_View());
  Templates.LoadLocalTemplates("");
  Doc.Run(elem,a);
  !function(t)
  {
   ASCIICamera.UserMedia();
   navigator.getUserMedia(t[0],t[1],t[2]);
  }?error.Set("Your browser does not support this feature!"):Camera.Start(function(v)
  {
   Application.Draw(v);
  },Camera.InitVideoStream(Application.video().elt,function()
  {
   error.Set("No camera was found!");
  }));
 };
 Application.video=function()
 {
  SC$3.$cctor();
  return SC$3.video;
 };
 Application.Draw=function(v)
 {
  var video;
  video=v.Video;
  Application.canvas().elt.getContext("2d").drawImage(video,0,0,Global.Number(video.width),Global.Number(video.height));
  Application.txt().Set(Ascii.AsciiFromCanvas(Application.canvas().elt));
 };
 Application.style=function()
 {
  SC$3.$cctor();
  return SC$3.style;
 };
 Application.txt=function()
 {
  SC$3.$cctor();
  return SC$3.txt;
 };
 Application.canvas=function()
 {
  SC$3.$cctor();
  return SC$3.canvas;
 };
 ...
 SC$2.$cctor=function()
 {
  ...
  SC$2.javascriptNode=MinMaxTimeDomain.context().createScriptProcessor(1024,1,1);
  SC$2.CanvasEl=Doc.Element("canvas",[AttrProxy.Create("width","800"),AttrProxy.Create("height","256"),AttrProxy.Create("style","background-color: black;")],[]);
  SC$2.column=0;
  SC$2.um=navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;
  SC$2.Sample=Samples.Build().Id("MinMaxTimeDomain").FileName("MinMaxTimeDomain.fs").Keywords(List.ofArray(["webrtc","visualizer","time domain","microphone"])).Render(function(e)
  {
   MinMaxTimeDomain.Main(e);
  }).Create();
 };
 SC$3.$cctor=function()
 {
  SC$3.$cctor=Global.ignore;
  SC$3.UserMedia=navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;
  SC$3.width=120;
  SC$3.height=90;
  SC$3.characters=" .,:;i1tfLCG08@";
  SC$3.video=Doc.Element("video",[AttrProxy.Create("width",Global.String(ASCIICamera.width())),AttrProxy.Create("height",Global.String(ASCIICamera.height()))],[]);
  SC$3.canvas=Doc.Element("canvas",[AttrProxy.Create("width",Global.String(ASCIICamera.width())),AttrProxy.Create("height",Global.String(ASCIICamera.height()))],[]);
  SC$3.style="font-family: 'Courier New', 'Courier', monospace;font-size: 10px;line-height: 10px;";
  SC$3.txt=Var$1.Create$1("");
  SC$3.Sample=Samples.Build().Id("ASCIICamera").FileName("ASCIICamera.fs").Keywords(List.ofArray(["webrtc","camera","video","effects"])).Render(function(e)
  {
   Application.Main(e);
  }).Create();
 };
 ...
 ASCIICamera.width=function()
 {
  SC$3.$cctor();
  return SC$3.width;
 };
 ASCIICamera.height=function()
 {
  SC$3.$cctor();
  return SC$3.height;
 };
 ASCIICamera.SetupRequestAnimFrame=function()
 {
  ...
  Global.cancelAnimFrame=function()
  {
   return Global.cancelAnimationFrame||Global.mozCancelAnimationFrame||Global.clearTimeout;
  }();
 };
 ASCIICamera.UserMedia=function()
 {
  SC$3.$cctor();
  return SC$3.UserMedia;
 };
 ...
 AudioHolder.StopCurrent=function()
 {
  ...
  o==null?void 0:o.$0.disconnect();
  o$1=AudioHolder.jsNode();
  o$1==null?void 0:o$1.$0.disconnect();
 };
 AudioHolder.currentPlaying=function()
 {
  SC$8.$cctor();
  return SC$8.currentPlaying;
 };
 AudioHolder.jsNode=function()
 {
  SC$8.$cctor();
  return SC$8.jsNode;
 };
 AudioHolder.SetCurrent=function(src,jsnode)
 {
  AudioHolder.set_currentPlaying({
   $:1,
   $0:src
  });
  AudioHolder.set_jsNode({
   $:1,
   $0:jsnode
  });
 };
 ...
 {
  SC$8.$cctor();
  SC$8.currentPlaying=$1;
 };
 AudioHolder.set_jsNode=function($1)
 {
  SC$8.$cctor();
  SC$8.jsNode=$1;
 };
 ...
 Camera.InitVideoStream=function(video,onNotSupprted)
 {
  ASCIICamera.UserMedia();
  navigator.getUserMedia({
   video:true,
  ...
  {
   video.src=Global.URL&&URL.createObjectURL(stream)||stream;
  },onNotSupprted);
  return Video.New(video,null,false);
 };
 Camera.Start=function(draw,video)
 {
  video.Video.play();
  return Video.New(video.Video,{
   $:1,
   $0:Global.requestAnimFrame(function()
   {
    Camera.FrameFun(video,draw,void 0);
   })
  },true);
 };
 Camera.FrameFun=function(video,draw,u)
 {
  draw(video);
  Global.requestAnimFrame(function()
  {
 ...
 Video.New=function(Video$1,Handle,Playing)
 {
  return{
   Video:Video$1,
   Handle:Handle,
 ...
 Ascii.AsciiFromCanvas=function(canvas)
 {
  var x,y,result,ctx,width,height,contrast,contrastFactor,image,offset,color,contrastedColor;
  ...
  while(y<height)
   {
    ...
    while(x<width)
     {
      offset=(y*width+x)*4;
      color=Ascii.GetColorAtOffset(image.data,offset);
      contrastedColor=Color.New(Ascii.Bound(Operators.toInt(Math.floor((color.Red-128)*contrastFactor)+128),0,255),Ascii.Bound(Operators.toInt(Math.floor((color.Green-128)*contrastFactor)+128),0,255),Ascii.Bound(Operators.toInt(Math.floor((color.Blue-128)*contrastFactor)+128),0,255),color.Alpha);
      result=result+(Ascii.characters())[Ascii.characters().length-1-Operators.toInt(Math.round((0.299*contrastedColor.Red+0.587*contrastedColor.Green+0.114*contrastedColor.Blue)/255*(Ascii.characters().length-1)))];
      x=x+1;
     }
    result=result+"\n";
    y=y+2;
 ...
 Ascii.GetColorAtOffset=function(data,offset)
 {
  return Color.New(Arrays.get(data,offset),Arrays.get(data,offset+1),Arrays.get(data,offset+2),Arrays.get(data,offset+3));
 };
 Ascii.Bound=function(value,minn,maxx)
 {
  var b;
  b=Unchecked.Compare(maxx,value)===-1?maxx:value;
  return Unchecked.Compare(minn,b)===1?minn:b;
 };
 Ascii.characters=function()
 {
  SC$3.$cctor();
  return SC$3.characters;
 };
 SC$6.$cctor=function()
 {
 ...
 SC$8.$cctor=function()
 {
  ...
  SC$8.currentPlaying=null;
  SC$8.jsNode=null;
 };
 Color.New=function(Red,Green,Blue,Alpha)
 {
  return{
   Red:Red,
   Green:Green,
 ...
 SC$11.$cctor=function()
 {
  ...
  SC$11.noneCT=CT.New(false,[]);
  SC$11.scheduler=new Scheduler.New();
  SC$11.defCTS=[new CancellationTokenSource.New()];
  SC$11.Zero=Concurrency.Return();
  SC$11.GetCT=function(c)
  {
   c.k({
    $:0,
    $0:c.ct
