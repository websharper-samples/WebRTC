...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,Site,ASCIICamera,Application,Number,Ascii,AudioHolder,jQuery,navigator,Camera,Samples,WebSharper,List,Html,Client,Default,T,Math,String,Operators,IntrinsicFunctionProxy,window,Option,Set,MinMaxTimeDomain,Client1,Uint8Array,Seq,Builder,Sample,document,Operators1,Unchecked,EventsPervasives;
 Runtime.Define(Global,{
  Site:{
   ASCIICamera:{
    Application:{
     Draw:function(v)
     {
      var video;
      video=v.Video;
      Application.canvas().get_Body().getContext("2d").drawImage(video,0,0,Number(video.width),Number(video.height));
      return Application.asciiContainer().set_Html(Ascii.AsciiFromCanvas(Application.canvas().get_Body()));
     },
     Main:function(elem)
     {
      var _this,objectArg,arg001,onNotSupprted;
      AudioHolder.StopCurrent();
      ASCIICamera.SetupRequestAnimFrame();
      jQuery(elem).append(Application.asciiContainer().Dom);
      if(!Runtime.Tupled(function(tupledArg)
      {
       ...
       return navigator.getUserMedia(arg00,arg01,arg02);
      }))
       {
        Application.error().set_Text("Your browser does not support this feature!");
        _this=Application.asciiContainer();
        objectArg=_this["HtmlProvider@33"];
        arg001=_this.get_Body();
        objectArg.SetCss(arg001,"display","none");
        return jQuery(elem).append(Application.error().Dom);
       }
      else
       {
        onNotSupprted=function()
        {
         var _this1,objectArg1,arg00;
         _this1=Application.asciiContainer();
         objectArg1=_this1["HtmlProvider@33"];
         arg00=_this1.get_Body();
         objectArg1.SetCss(arg00,"display","none");
         Application.error().set_Text("No camera was found!");
         return jQuery(elem).append(Application.error().Dom);
        };
        Camera.Start(function(v)
        {
         return Application.Draw(v);
        },Camera.InitVideoStream(Application.video().get_Body(),onNotSupprted));
        return;
       }
     },
     Sample:Runtime.Field(function()
     {
      return Samples.Build().Id("ASCIICamera").FileName("ASCIICamera.fs").Keywords(List.ofArray(["webrtc","camera","video","effects"])).Render(function(elem)
      {
       return Application.Main(elem);
      }).Create();
     }),
     asciiContainer:Runtime.Field(function()
     {
      var _this,x;
      _this=Default.Attr();
      x=Application.style();
      return Default.Pre(List.ofArray([_this.NewAttr("style",x)]));
     }),
     canvas:Runtime.Field(function()
     {
      var x,arg00,_this,arg001,_this1,_this2;
      arg00=Global.String(ASCIICamera.width());
      _this=Default.Attr();
      arg001=Global.String(ASCIICamera.height());
      _this1=Default.Attr();
      x=List.ofArray([_this.NewAttr("width",arg00),_this1.NewAttr("height",arg001)]);
      _this2=Default.Tags();
      return _this2.NewTag("canvas",x);
     }),
     error:Runtime.Field(function()
     {
     ...
     {
      return"font-family: 'Courier New', 'Courier', monospace;font-size: 10px;line-height: 10px;";
     }),
     video:Runtime.Field(function()
     {
      var x,arg00,_this,arg001,_this1,_this2;
      arg00=Global.String(ASCIICamera.width());
      _this=Default.Attr();
      ...
      x=List.ofArray([_this.NewAttr("width",arg00),_this1.NewAttr("height",arg001)]);
      _this2=Default.Tags();
      return _this2.NewTag("video",x);
     })
    },
    Ascii:{
     AsciiFromCanvas:function(canvas)
     {
      var ctx,width,height,contrastFactor,image,x,y,result,offset,color,contrastedColor,brightness;
      ctx=canvas.getContext("2d");
      width=canvas.width;
      height=canvas.height;
      contrastFactor=259*(128+255)/(255*(259-128));
      image=ctx.getImageData(0,0,Number(width),Number(height));
      x=0;
      y=0;
      result="";
      while(y<height)
        ...
        while(x<width)
         {
          ...
          contrastedColor={
           Red:Ascii.Bound(Math.floor(Number(color.Red-128)*contrastFactor)+128<<0,0,255),
           Green:Ascii.Bound(Math.floor(Number(color.Green-128)*contrastFactor)+128<<0,0,255),
           Blue:Ascii.Bound(Math.floor(Number(color.Blue-128)*contrastFactor)+128<<0,0,255),
           Alpha:color.Alpha
          };
          brightness=(0.299*Number(contrastedColor.Red)+0.587*Number(contrastedColor.Green)+0.114*Number(contrastedColor.Blue))/255;
          result=result+String.fromCharCode(Ascii.characters().charCodeAt(Ascii.characters().length-1-(Math.round(brightness*Number(Ascii.characters().length-1))<<0)));
          x=x+1;
         }
        result=result+"\n";
        y=y+2;
     ...
     Bound:function(value,minn,maxx)
     {
      return Operators.Max(minn,Operators.Min(maxx,value));
     },
     GetColorAtOffset:function(data,offset)
     {
      return{
       Red:IntrinsicFunctionProxy.GetArray(data,offset),
       Green:IntrinsicFunctionProxy.GetArray(data,offset+1),
       Blue:IntrinsicFunctionProxy.GetArray(data,offset+2),
       Alpha:IntrinsicFunctionProxy.GetArray(data,offset+3)
      };
     },
     characters:Runtime.Field(function()
     {
      return" .,:;i1tfLCG08@";
     })
    },
    Camera:{
     FrameFun:function(video,draw)
     {
      draw(video);
      window.requestAnimFrame(function(_arg20_)
      {
       return Camera.FrameFun(video,draw,_arg20_);
      });
      return;
     },
     InitVideoStream:function(video,onNotSupprted)
     {
      ASCIICamera.UserMedia();
     ...
     {
      Option.iter(function(id)
      {
       return window.cancelAnimFrame(id);
      },video.Handle);
      video.Video.pause();
      return{
       Video:video.Video,
     ...
     Start:function(draw,video)
     {
      ...
      return{
       ...
       Handle:{
        $:1,
        $0:window.requestAnimFrame(function(_arg20_)
        {
         return Camera.FrameFun(video,draw,_arg20_);
        })
       },
     ...
     {
      var v;
      v=Camera.Pause(video);
      v.Video.src="";
      return v;
     }
    },
    SetupRequestAnimFrame:function()
    {
     var $0=this,$this=this;
     Global.window.requestAnimFrame=function()
     {
      return Global.window.requestAnimationFrame||Global.window.webkitRequestAnimationFrame||Global.window.mozRequestAnimationFrame||function(callback)
      {
       Global.window.setTimeout(callback,1000/60);
      };
     }();
     Global.window.cancelAnimFrame=function()
     {
      return Global.window.cancelAnimationFrame||Global.window.mozCancelAnimationFrame||Global.window.clearTimeout;
     }();
    },
    UserMedia:Runtime.Field(function()
    {
     var _;
     _=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;
     navigator.getUserMedia=_;
     return _;
    }),
   ...
   AudioHolder:{
    SetCurrent:function(src,jsnode)
    {
     var _,_1;
     _={
      $:1,
   ...
   Client:{
    All:Runtime.Field(function()
    {
     ...
     return Set.Create(List.ofArray([_op_BangPlus_8(MinMaxTimeDomain.Sample()),_op_BangPlus_8(Application.Sample())]));
    }),
   ...
   MinMaxTimeDomain:{
    Analyser:function(stream)
    {
     ...
     MinMaxTimeDomain.javascriptNode().onaudioprocess=function()
     {
      ...
      return MinMaxTimeDomain.DrawTimeDomain(MinMaxTimeDomain.CanvasEl().Dom.getContext("2d"),array);
     };
     ...
     return analyser.connect(MinMaxTimeDomain.javascriptNode());
    },
    CanvasEl:Runtime.Field(function()
    {
     var x,_this,_this1;
     _this=Default.Attr();
     x=List.ofArray([Default.Width("800"),Default.Height("256"),_this.NewAttr("style","background-color: black;")]);
     _this1=Default.Tags();
     return _this1.NewTag("canvas",x);
    }),
    DrawTimeDomain:function(ctx,array)
    {
     var c,value,width,value1,height,minValue,maxValue,action,list,max,min,_;
     c=jQuery(MinMaxTimeDomain.CanvasEl().Dom);
     value=c.width();
     width=Number(value);
     value1=c.height();
     ...
     action=function()
     {
      return function(a)
      {
       var value2;
       value2=Number(a)/256;
       if(value2>maxValue.contents)
        {
         maxValue.contents=value2;
        }
       return value2<minValue.contents?void(minValue.contents=value2):null;
      };
     };
     list=MinMaxTimeDomain.ToList(array);
     Seq.iteri(action,list);
     max=height-height*maxValue.contents-1;
     min=height-height*minValue.contents-1;
     ctx.fillStyle="#ffffff";
     ctx.fillRect(MinMaxTimeDomain.column(),min,1,max-min);
     _=MinMaxTimeDomain.column()+1;
     MinMaxTimeDomain.column=function()
     {
      return _;
     };
     if(MinMaxTimeDomain.column()>=width)
      {
       MinMaxTimeDomain.column=function()
       {
        return 0;
    ...
    LoadSound:function(onNotSupported)
    {
     MinMaxTimeDomain.um();
     ...
     {
      return MinMaxTimeDomain.Analyser(stream);
     },onNotSupported);
    },
    Main:function(elem)
    {
     var error;
     AudioHolder.StopCurrent();
     error=Default.Div(Runtime.New(T,{
      $:0
     }));
     if(!Runtime.Tupled(function(tupledArg)
     {
      ...
      {
       error.set_Text("Your browser does not support this feature!");
       jQuery(elem).append(error.Dom);
       return;
      }
     else
      {
       jQuery(elem).append(MinMaxTimeDomain.CanvasEl().Dom);
       return MinMaxTimeDomain.LoadSound(function()
       {
        var _this,objectArg,arg00;
        _this=MinMaxTimeDomain.CanvasEl();
        objectArg=_this["HtmlProvider@33"];
        arg00=_this.get_Body();
        objectArg.SetCss(arg00,"display","none");
        error.set_Text("No microphone was found!");
        jQuery(elem).append(error.Dom);
        return;
       });
      }
    ...
    Sample:Runtime.Field(function()
    {
     return Samples.Build().Id("MinMaxTimeDomain").FileName("MinMaxTimeDomain.fs").Keywords(List.ofArray(["webrtc","visualizer","time domain","microphone"])).Render(function(elem)
     {
      return MinMaxTimeDomain.Main(elem);
     }).Create();
    }),
    ...
    javascriptNode:Runtime.Field(function()
    {
     return MinMaxTimeDomain.context().createScriptProcessor(1024,1,1);
    }),
    um:Runtime.Field(function()
    {
     var _;
     _=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||navigator.msGetUserMedia;
     navigator.getUserMedia=_;
   ...
   Samples:{
    ...
    Sample:Runtime.Class({
     Show:function()
     {
      ...
      url="http://github.com/intellifactory/websharper.webrtc/blob/master/Site/"+this.FileName;
 ...
 Runtime.OnInit(function()
 {
  ...
  ASCIICamera=Runtime.Safe(Site.ASCIICamera);
  Application=Runtime.Safe(ASCIICamera.Application);
  Number=Runtime.Safe(Global.Number);
  Ascii=Runtime.Safe(ASCIICamera.Ascii);
  AudioHolder=Runtime.Safe(Site.AudioHolder);
  jQuery=Runtime.Safe(Global.jQuery);
  navigator=Runtime.Safe(Global.navigator);
  Camera=Runtime.Safe(ASCIICamera.Camera);
  ...
  MinMaxTimeDomain=Runtime.Safe(Site.MinMaxTimeDomain);
 ...
 {
  MinMaxTimeDomain.um();
  MinMaxTimeDomain.javascriptNode();
  MinMaxTimeDomain.context();
  MinMaxTimeDomain.column();
  MinMaxTimeDomain.Sample();
  MinMaxTimeDomain.CanvasEl();
  Client1.Main();
  Client1.All();
  AudioHolder.jsNode();
  AudioHolder.currentPlaying();
